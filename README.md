...
2.  **Python Worker (`docker` container):** Tails the export files (vitals JSON + four waveform CSVs), publishes each sample to a per-signal MQTT topic, batch-inserts everything into TimescaleDB, and serves historic data over FastAPI on port **8001**.
...
The worker publishes to `mp50/VitalSigns` (numerics), `mp50/HF-ECG`, `mp50/HF-EEG`, `mp50/HF-PLETH`, and `mp50/HF-RESP` (waveforms). Every payload is normalized JSON:
